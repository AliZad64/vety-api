...
class RegistrationForm(UserCreationForm):
    ...
    """
    phone_number = forms.CharField(max_length=60, help_text='Required. Add a valid email address')
...
class AccountUpdateForm(forms.ModelForm):
    ...
    def clean_email(self):
        if self.is_valid():
            ...
            try:
                account = EmailAccount.objects.exclude(pk=self.instance.pk).get(phone_number=phone_number)
            except EmailAccount.DoesNotExist:
                return phone_number
            raise forms.ValidationError("Email '%s' already in use." % phone_number)
